...
const UserHeder = ({user}) => {
  ...
  return (
    <VStack gap={4} alignItems={"start"}>
     ...
     {currentUser?._id === user._id && (   // login user(current user) and params value(user) is same update button is visible
       <Link as={RouterLink} to='/update'>
        <Button size={"sm"}>Update Profile</Button>
       </Link>
        )}

    {currentUser?._id !== user._id &&     // following functionality
